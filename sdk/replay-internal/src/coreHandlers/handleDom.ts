...
export const handleDomListener: (
  replay: ReplayContainer,
) => (handlerData: HandlerDataDom) => void = (replay: ReplayContainer) => {
...
export function getBaseDomBreadcrumb(
  target: Node | null,
  message: string,
): Breadcrumb {
            ...
            textContent: Array.from(element.childNodes)
              .map(
                (node: serializedNodeWithId) =>
                  node.type === NodeType.Text && node.textContent,
              )
...
function getDomTarget(handlerData: HandlerDataDom): {
  target: Node | null;
  message: string;
} {
  const isClick = handlerData.name === 'click';
  ...
  try {
    target = isClick
      ? getClickTargetNode(handlerData.event as Event)
      : getTargetNode(handlerData.event as Event);
...
function isElement(
  node: serializedNodeWithId,
): node is serializedElementNodeWithId {
