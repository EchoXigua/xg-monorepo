 ...
 *
 * 在计算机系统中，流常用于处理如下场景：
 * - 文件读写：逐步读取大文件，而不是一次性加载整个文件
 * - 网络请求：服务器逐步发送响应，而不是一次性发送全部数据
 * - 音视频处理：播放音视频时，逐步加载文件，而非一次性加载整个媒体文件
 *
 * Stream 的主要优点
 * 1. 节省内存：因为流不会一次性加载所有数据，它能处理非常大的数据集，且内存占用相对较少
 * 2. 实时处理：流可以开始处理接收到的数据，而不必等待整个数据集传输完成。
 * 这对于数据传输（如网络或文件传输）非常重要
 * 3. 逐步消费：数据在流入时就可以被消费处理，不需要全部数据都准备好
 *
 * 流处理中的一个重要概念是“结束”，也就是不再有数据流入。
 * 流在处理完成所有数据时，我们需要通过特殊标志来通知流已经结束
 */
