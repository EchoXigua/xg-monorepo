import type { Client, Scope as ScopeInterface } from '@xigua-monitor/types';
import { isThenable } from '@xigua-monitor/utils';
import {
...
function withIsolationScope<T>(
  callback: (isolationScope: ScopeInterface) => T,
): T {
...
export function getStackAsyncContextStrategy(): AsyncContextStrategy {
  return {
    ...
    withSetIsolationScope: <T>(
      _isolationScope: ScopeInterface,
      callback: (isolationScope: ScopeInterface) => T,
    ) => {
